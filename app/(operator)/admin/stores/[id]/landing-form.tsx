    ...
    <div className="rounded-lg border bg-card p-5 space-y-4">
      ...
      <div className="rounded-lg border-2 border-amber-200 bg-amber-50/40 p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-foreground">Generate ด้วย AI</h3>
        </div>
          ...
          <div className="flex items-start gap-3 rounded-md border border-amber-300 bg-amber-50 px-3 py-3 text-sm text-amber-900">
            ...
            <div className="flex-1">
              ...
              <Link
                ...
                className="mt-2 inline-flex items-center gap-1 rounded-md border border-amber-400 bg-card px-2.5 py-1 text-xs font-medium text-amber-800 hover:bg-amber-100"
              >
                + เพิ่มสินค้า
              </Link>
            </div>
        ...
        {props.hasLandingPage && engine === "managed" && (
          <div className="flex flex-wrap items-center gap-3 border-t border-border pt-3">
            ...
            <button
              ...
              disabled={isGenerating}
              className="ml-auto inline-flex items-center gap-1.5 rounded-md border-2 border-amber-500 bg-card px-4 py-1.5 text-sm font-bold text-amber-700 hover:bg-amber-50 disabled:opacity-50"
            >
              {isGenerating ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin" />
        ...
        <div className="mt-3 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <button
              ...
              onClick={handleClear}
              disabled={busy !== null}
              className="inline-flex items-center gap-1.5 rounded-md border border-destructive/30 bg-card px-3 py-2 text-sm text-destructive hover:bg-destructive/10 disabled:opacity-50"
              title={
                isGenerating
            ...
            <button
              ...
              className="ml-auto inline-flex items-center gap-1.5 rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary/90 disabled:opacity-50"
            >
              {busy === "save" ? (
                <Loader2 className="h-4 w-4 animate-spin" />
              ) : null}
...
function RecoverFromSession({ storeId }: { storeId: string }) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="mt-2 inline-flex items-center gap-1 rounded-md border border-current/20 bg-card/60 px-2.5 py-1 text-[11px] font-medium hover:bg-card"
      >
  ...
  return (
    <div className="mt-2 space-y-2 rounded-md border border-current/20 bg-card/60 p-2">
      ...
      <div className="flex gap-1.5">
        <input
          ...
          className="flex-1 rounded-md border bg-card px-2 py-1 text-xs font-mono"
          disabled={busy}
        />
        <button
          type="button"
