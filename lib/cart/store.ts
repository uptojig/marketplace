...
// Global multi-store cart, persisted to localStorage. Selection is a
// distinct concept from items in the cart — the cart can hold items
// from multiple stores and the user toggles which ones go to checkout.
// Persist version 2 because the schema swapped a coupons[] field for
// appliedCouponIds (just IDs — the calculator re-fetches by id at render).
    ...
    (set, get) => ({
      ...
      deselectAllInStore: (storeId) =>
        set((state) => {
          ...
          return {
            selectedIds: state.selectedIds.filter((sid) => !drop.has(sid)),
          };
        }),
      ...
      removeCoupon: (couponId) =>
        set((state) => ({
          appliedCouponIds: state.appliedCouponIds.filter(
            (id) => id !== couponId,
          ),
        })),
