...
/**
 * Thai/THB bespoke PDP — client-side implementation.
 *
 * Used by `makePdpAdapter` in `./pdp-adapter` (a server-safe factory)
 * via a thin wrapper. Splitting the hooks/JSX into this file keeps the
 * factory module free of the `'use client'` directive so the
 * `pages.pdp: makePdpAdapter('06', '02')` call site inside the
 * server-side `lib/templates/registry.ts` doesn't get a client
 * reference proxy in place of a real function.
 *
 * `style` is computed by the factory (palette → CSS vars) and threaded
 * through as a prop so the same palette object the theme passed at
 * registration time tints the layout consistently.
 */
...
interface CoreProps extends ProductDetailProps {
  style?: React.CSSProperties;
}

export function PdpAdapterCore({ store, product, related, style }: CoreProps) {
