      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <nav className="hidden md:flex items-center gap-8 font-[family:var(--font-prompt)]">
            <Link href={`/stores/${store.slug}`} className="text-sm font-medium text-[#0f172a] hover:text-[#0284c7] transition-colors">หน้าหลัก</Link>
            <div className="group relative">
              ...
              <div className="absolute top-full left-0 mt-4 w-64 bg-white border border-[#cbd5e1] shadow-xl rounded-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                {categories.map((cat) => (
                  <Link key={cat.id} href={`/stores/${store.slug}/category/${cat.slug}`} className="block px-4 py-3 text-sm text-[#0f172a] hover:bg-[#e2e8f0] hover:text-[#0284c7] border-b border-[#cbd5e1] last:border-0 font-medium">
                    {cat.name}
                  </Link>
                ))}
              </div>
            </div>
            <Link href={`/stores/${store.slug}/requests`} className="text-sm font-medium text-[#0f172a] hover:text-[#0284c7] transition-colors flex items-center gap-1.5">
